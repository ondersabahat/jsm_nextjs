...
const Profile = async ({ params, searchParams }: RouteParams) => {
  ...
  const [
    { success: userQuestionsSuccess, data: userQuestions, error: userQuestionsError },
    ...
    getUserQuestions({ userId: id, page: Number(page) || 1, pageSize: Number(pageSize) || 10 }),
    getUserAnswers({ userId: id, page: Number(page) || 1, pageSize: Number(pageSize) || 10 }),
            ...
            <DataRenderer
              ...
              render={(tags) => (
                <div className="mt-3 flex w-full flex-col gap-4">
                  {tags.map((tag) => (
                    <TagCard key={tag._id} _id={tag._id} name={tag.name} questions={tag.count} compact showCount />
                  ))}
                </div>
              )}
            />
